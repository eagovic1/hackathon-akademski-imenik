    ...
    if (prompt.trim().length <= 2) {
        ...
        axios.post(url, body).then(response => {
            ...
            try {
                ...
                if (text.includes('keywords" :') || text.includes('keywords":')) {
                    ...
                    if (text.includes('error":') || text.includes('error" :')) {
                        res.status(400).json({ error: "Invalid search input! Please enter a valid search input." })
                    }
                }
            }
        })
